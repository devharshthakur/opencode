...
2. **Isolate or resume the work.** Inspect the current branch, status, relevant diffs, local `main`, existing branches and worktrees, and any PR for this issue before any Git mutation. Preserve all pre-existing changes; do not stash, reset, clean, or switch a dirty checkout. On a repeat run, reuse the matching issue branch and linked worktree if they belong to this repository, are clean, and agree with the remote PR head when one exists. If the branch exists without a worktree, attach a new worktree to it only after verifying its identity and history; never reset or replace it. If an existing issue worktree is dirty, or its local/remote state is unclear, leave it intact and report the blocker instead of creating a competing branch. When there is no existing issue branch, create a uniquely named `issue/<number>-<short-slug>` branch at the local `main` commit in a separate Git worktree, using an unused sibling path. Do not silently base on the current branch or fetch/rebase/update `main`; report if local `main` is missing or stale relative to the remote. Git operations must pass the active tool permission checks: request approval when required, never work around a denial, and stop with a clear blocker if permission is unavailable. If available, use `execute` to call `opencode.session_move` after choosing the worktree; wait for the move to take effect before reading or editing there. Otherwise use explicit paths inside the worktree for file tools and set `shell.workdir` to it, honoring external-directory permissions. Confirm branch, base commit, and clean status in the selected worktree before implementation.
...
7. **Open or update the PR for changes.** The user's issue request includes pushing the issue branch and creating a PR; do not ask a separate discretionary push or PR question. Respect tool/harness approval for every Git or GitHub mutation, and never retry a denial through another command or tool. After commits and push, verify the actual branch diff against `main` and check for an existing PR for the head branch. If one already exists, use it and verify that it contains the new commits; otherwise open one PR targeting `main` with the verified head branch (for example, `gh pr create --base main --head <branch> --title ... --body ...`). Do not merge it or prematurely close the issue. For a new PR, write a natural, concise title and body like a human reviewer would: explain the issue, the logical fix and why it works, include only code details necessary to understand the approach, note meaningful review findings if this was already implemented, and state the tests actually run plus limitations. Reference the issue in the body without implying it is already closed; avoid file-by-file inventories, generic boilerplate, inflated claims, and automatic commit-message summaries. Confirm the PR URL and head/base. If delivery is blocked, do not fabricate a diff or an empty PR; report why no PR could be created.

8. **Keep or explicitly clean up the worktree.** Keep the linked worktree after opening a PR so review feedback or CI fixes can resume on the same branch; report its path and branch. If the user explicitly asks to clean up this issue's worktree (including a later `cleanup` request), first identify the exact linked worktree, confirm it is not the main worktree, inspect its tracked, untracked, and ignored files, and verify that its branch has no unpushed commits relative to the correct remote branch. Never discard local changes or remove a worktree with uncertain state. Move the active session away from that worktree before removal; if this cannot be done safely, stop and explain. With tool/harness approval, run `git worktree remove <verified path>` without `--force`, then confirm its absence from `git worktree list`. Keep the branch and remote PR intact unless separately requested; do not automatically remove a worktree just because a PR was created.
